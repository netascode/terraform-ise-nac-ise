...
locals {
  ...
  network_access_conditions_circular_managed_names = toset([
        ...
        children = length(try(c3.children, [])) == 0 ? null : [for c4 in try(c3.children, []) : {
          ...
          children = length(try(c4.children, [])) == 0 ? null : [for c5 in try(c4.children, []) : {
            ...
            children = length(try(c5.children, [])) == 0 ? null : [for c6 in try(c5.children, []) : {
              ...
              attribute_value  = try(c6.attribute_value, local.defaults.ise.network_access.policy_elements.conditions.attribute_value, null)
              dictionary_name  = try(c6.dictionary_name, local.defaults.ise.network_access.policy_elements.conditions.dictionary_name, null)
              dictionary_value = try(c6.dictionary_value, local.defaults.ise.network_access.policy_elements.conditions.dictionary_value, null)
              condition_type   = try(c6.type, local.defaults.ise.network_access.policy_elements.conditions.type, null)
...
locals {
  network_access_policy_sets = [
    for generated_rank, ps in try(local.ise.network_access.policy_sets, []) : {
      ...
      condition_id               = contains(local.known_conditions_network_access, try(ps.condition.name, "")) ? local.network_access_all_condition_ids[ps.condition.name] : try(data.ise_network_access_condition.network_access_condition[ps.condition.name].id, null)
  ...
  network_access_authentication_rules = flatten([
    for ps in try(local.ise.network_access.policy_sets, []) : [
      for generated_rank, rule in try(ps.authentication_rules, []) : {
        ...
        condition_id               = contains(local.known_conditions_network_access, try(rule.condition.name, "")) ? local.network_access_all_condition_ids[rule.condition.name] : try(data.ise_network_access_condition.network_access_condition[rule.condition.name].id, null)
